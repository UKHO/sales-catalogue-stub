...
namespace PostManTests
{
...

    {
...

        {
            ...
            //ToDO Add Product URI to Tests
            var getProductUri = $"{configuration.SiteBaseUrl}/v1/productData/AVCS/";
            var getProductUriParams = "products?sinceDateTime=";
            sinceDateTime = DateTime.Now - TimeSpan.FromDays(14);
            productUri = $"{getProductUri}{getProductUriParams}{sinceDateTime.ToString("yyyy-MM-dd")}";
        ...
        {
            //ToDo Create Method for lines 77-79
            string tokenresult = _newaccesstoken.GetAcessToken();
            GetProductApiCall newuri = new GetProductApiCall();
            (ProductResponse ProductResponse, HttpStatusCode StatusCode) uriresult =
                await newuri.GetProductHttpClientAsync(tokenresult, productUri);
        ...
        public async Task Test_The_Get_Product_Request_With_No_Dates_Returns_Bad_Request()
        {
            ...
            GetProductApiCall newuri = new GetProductApiCall();
            (ProductResponse ProductResponse, HttpStatusCode StatusCode) uriresult =
                await newuri.GetProductHttpClientAsync(tokenresult, $"{configuration.SiteBaseUrl}/v1/productData/AVCS/products?sinceDateTime=");
        ...
        public async Task Test_The_Get_Product_Request_With_Incorrect_Date_Format_Returns_Bad_Request()
        {
            ...
            GetProductApiCall newuri = new GetProductApiCall();
            (ProductResponse ProductResponse, HttpStatusCode StatusCode) uriresult =
                await newuri.GetProductHttpClientAsync(tokenresult, $"{configuration.SiteBaseUrl}/v1/productData/AVCS/products?sinceDateTime=BLAH");
            uriresult.StatusCode.Should().Be(HttpStatusCode.BadRequest);
        }
    ...
    //Test to do Test_The_Get_Product_With_URI_sinceDateTime_In_The_Future_Returns_No_Products()
    //uriresult.StatusCode.Should().Be(HttpStatusCode.OK);
    //uriresult.ProductResponse.products.Count.Should().Be(0);
}